...
bool Name_pairs::operator==(const Name_pairs& other) {
	if (this->_names.size() == other._names.size() &&
		this->_ages.size() == other._ages.size())
	{
		for (decltype(this->_names.size()) i(0); i < this->_names.size(); ++i)
			if (this->_names[i] != other._names[i] ||
				this->_ages[i] != other._ages[i])
				return false;
		return true;
	}
	else return false;
...
std::ostream& operator<<(std::ostream& os, const Name_pairs& np) {
	for (decltype(np._names.size()) i(0); i < np._names.size(); ++i) {
		os << np._names[i] << " " << np._ages[i] << std::endl;
	}
	return os;
}